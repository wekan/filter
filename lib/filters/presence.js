 ...
 */
class PresenceFilter extends FilterSring {
  /**
   * @param {object} input
   * @param {string} input.attribute The name of the attribute this filter
   * targets.
   *
   ...
   * @param {boolean} [strictAttributeCase=true] If `false`, "fOo" witll mach
   * "foo".
   *
  ...
  static parse (buffer) {
    ...
    const attribute = reader.buffer.slice(0, reader.length).toString('utf-8').toLowerCase()
