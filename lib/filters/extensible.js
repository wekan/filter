...
const FilterSring = require('../filter-string')
 ...
 */
class ExtensibleFilter extends FilterSring {
  #dnAttributes
  #rule

  /**
   * @param {object} input
   * @param {string|undefined} [input.attribute=''] Name of the attribute to
   * match against, if any.
   * @param {*} [input.value=''] Value to test for.
   * @param {string} [input.rule] A matching rule OID if a speficic matching
   * rule is to be used.
   * @param {string|undefined} [input.matchType=''] An alias for `attribute`.
   * This parameter is provided for backward compatibility. Use `attribute`
   * instead.
   * @param {boolean} [input.dnAttributes=false] Indicates if all attributes
