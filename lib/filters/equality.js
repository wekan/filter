...
const { Ber, BerReader } = require('@ldapjs/asn1')
...
class EqualityFilter extends FilterString {
  ...
  /**
   * @param {object} input
   * @param {string} input.attribute The name of the LDAP addtribute this
   * filter will target.
   * @param {string|Buffer} [input.value] A string or buffer value as the
   * test for this filter. Required if `raw` is not provided.
   * @param {Buffer} [input.raw] A buffer to use as the test for this filter.
  ...
  _toBer (ber) {
    ...
    ber.writeBuffer(this.#raw, Ber.OctetString)
    return ber
  }
  ...
  static parse (buffer) {
    ...
    let value = reader.readString(Ber.OctetString, true)

    if (this.attribute === 'objectclass') value = value.toLowerCase()
